...
def write_into_tfrecord_v2(file_path_in, tf_writer):
    with tf.gfile.Open(file_path_in, 'rb') as f:
        if sys.version_info < (3,):
            data = cPickle.load(f)  # python2
        else:
    ...
    for j in range(num_images):
        ...
        example = image_to_tfexample(
            image_raw, b'png', _IMAGE_SIZE, _IMAGE_SIZE, label)
        tf_writer.write(example.SerializeToString())
...
def cifar_to_tfrecord():
    ...
    file_path_train_out = os.path.join(input_dir, "train_v2.tfrecord")
    ...
    for index, name in enumerate(train_file_names):
        ...
        write_into_tfrecord_v2(file_path_in, tf_writer)
    tf_writer.close()
    file_path_test_out = os.path.join(input_dir, "test_v2.tfrecord")
    tf_writer_test = tf.python_io.TFRecordWriter(file_path_test_out)
    for index, name in enumerate(test_file_names):
        file_path_in = os.path.join(input_dir, name)
        write_into_tfrecord_v2(file_path_in, tf_writer_test)
