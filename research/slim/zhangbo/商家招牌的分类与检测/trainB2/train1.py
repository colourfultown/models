...
def trian():
    ...
    ip = '../createDataB/output/trainA6/'
    ipts = '../createDataB/output/testA3/'

    batch_size2 = 50
    tsn=int(1000 / batch_size2)
    ...
    imgs, labels = tf.train.shuffle_batch([img, label], batch_size=batch_size1, num_threads=10,
                                          capacity=1000 + 10 * batch_size1, min_after_dequeue=2000)
    # imgs, labels = tf.train.batch([img, label], batch_size=batch_size1, num_threads=4,capacity=3000 + 10 * batch_size1)
    ...
    tsimgs, tslabels = tf.train.batch([tsimg, tslabel], batch_size=batch_size2, num_threads=8,
                                              capacity=100 + 8 * batch_size2)
    # tsimgs, tslabels = tf.train.batch([tsimg, tslabel], batch_size=batch_size2, num_threads=1,capacity=500 + 4 * batch_size2)
    ...
    with slim.arg_scope(inception.inception_resnet_v2_arg_scope()):
        logits, _ = inception.inception_resnet_v2(tfimages, num_classes=N_CLASSES, is_training=True)
        # logits, _ = inception_v3.inception_v3(tfimages, num_classes=1000, is_training=True)
        # logits=slim.stack(logits, slim.fully_connected, [800, 500, N_CLASSES], scope='fc')
...
CKPT_FILE='../model/inception_resnet_v2_2016_08_30.ckpt'
TRAIN_FILE='model1/A6.ckpt'
STEPS=1000000
if __name__=='__main__':
    ...
    # 2970
    # train
    # loss: 0.41308122873306274, val
    # acc: 0.9720000058412552