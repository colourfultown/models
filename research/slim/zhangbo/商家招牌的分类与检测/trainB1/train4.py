...
def trian():
    ...
    imgs, labels = tf.train.shuffle_batch([img, label], batch_size=batch_size1, num_threads=16,
                                          capacity=3000 + 10 * batch_size1, min_after_dequeue=3000)
    ...
    tsfilename_qu = tf.train.string_input_producer(tsfiles, shuffle=True)
    tsimg, tslabel = read_de(tsfilename_qu)
    tsimgs, tslabels = tf.train.shuffle_batch([tsimg, tslabel], batch_size=batch_size2, num_threads=4,
                                              capacity=500 + 4 * batch_size2, min_after_dequeue=500)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i%20==0:
                ...
                validation_accuracy = sess.run(evaluation_step, feed_dict={tfimages: tsX, tflabels: tsY})
                print('{} train loss : {},test acc : {}\n'.format(i,loss,validation_accuracy))
        coord.request_stop()
        coord.join(threads)
