...
def trian():
    ...
    imgs, labels = tf.train.shuffle_batch([img, label], batch_size=batch_size1, num_threads=4,
                                          capacity=3000 + 10 * batch_size1, min_after_dequeue=3000)

    tsfiles = tf.train.match_filenames_once(ipts + 'data.tfrecords*')
    tsfilename_qu = tf.train.string_input_producer(tsfiles, shuffle=True,num_epochs = 10000000)
    tsimg, tslabel = read_de(tsfilename_qu)
    tsimgs, tslabels = tf.train.shuffle_batch([tsimg, tslabel], batch_size=batch_size2, num_threads=1,
