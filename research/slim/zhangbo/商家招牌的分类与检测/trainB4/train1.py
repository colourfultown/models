...
def trian():
    ...
    ip = '../createDataD/output/trainA0/'
    ipts = '../createDataD/output/testA0/'
    ...
    imgs, labels = tf.train.shuffle_batch([img, label], batch_size=batch_size1, num_threads=12,
                                          capacity=2000 + 12 * batch_size1, min_after_dequeue=2000)
    ...
    tfimages = tf.placeholder(tf.float32, [None, 224, 224, 3], name='input_images')
    tflabels = tf.placeholder(tf.int64, [None], name='labels')
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits, _ = resnet_v2.resnet_v2_152(tfimages, num_classes=N_CLASSES, is_training=True)
...
CKPT_FILE='../model/resnet_v2_152.ckpt'
TRAIN_FILE='model1/A5.ckpt'
STEPS=1000000
